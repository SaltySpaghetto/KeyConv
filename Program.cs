...
namespace ModelConverter
{
    class Program
    {
        ...
        public static MDL OpenMDL(string FileName)
        {
            MDL mdl = new MDL();
            File_Info file_Info = new File_Info();
            file_Info.FileName = Path.GetFileName(FileName);
            file_Info.FilePath = FileName;
            mdl.FileInfo = file_Info;
            mdl.Load(File.OpenRead(FileName));

            return mdl;
        }

        public static KEY OpenKEY(string FileName)
        {
            KEY key = new KEY();
            File_Info file_Info = new File_Info();
            file_Info.FileName = Path.GetFileName(FileName);
            file_Info.FilePath = FileName;
            key.FileInfo = file_Info;
            key.Load(File.OpenRead(FileName));
        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            if (true)
            {
                ...
                var importedMdl = OpenMDL(argsu[0]);
                var isMdl = importedMdl is MDL;
                if (isMdl)
                {
                    var importModel = importedMdl.ToGeneric();
                    importModel.OrderBones(importModel.Skeleton.Bones.OrderBy(x => GetBoneIndex(x.Name)).ToList());

                    var importedKey = OpenKEY(argsu[1]);

                    ExportAnimation((KEY)importedKey, importModel.Skeleton);
                }
            ...
            var importedModel = OpenMDL(target);//(IModelFormat)STFileLoader.OpenFileFormat(target);
        ...
        static void ExportAnimation(KEY model, STSkeleton skeleton)
        {
            string daePath = model.FileInfo.FileName.Replace(".key", ".smd");
            string folder = Path.GetFileNameWithoutExtension(model.FileInfo.FileName);
            string folderDir = Path.Combine(Path.GetDirectoryName(model.FileInfo.FileName), folder);
            if (!Directory.Exists($"{folder}"))
                Directory.CreateDirectory($"{folder}");

            SEANIM2.Save($"{daePath}", model, skeleton);

            Console.WriteLine($"Exported animation {model.FileInfo.FileName}!");
        }
    }
}
